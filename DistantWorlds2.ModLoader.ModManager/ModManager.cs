...
public class ModManager : IModManager {
  ...
  private Assembly? ModAssemblyResolver(object sender, ResolveEventArgs args) {
    try {
      ...
      Debug.WriteLine(asm is not null
        ? $"{asm.GetName().Name} is requesting {name}"
        : $"An unknown loader is requesting {name}");
      ...
      if (ctx is null) {
        var dir = Path.GetDirectoryName(new Uri(asm.EscapedCodeBase).LocalPath)!;
        Debug.WriteLine($"Checking if Mod Directory: {dir} vs. {_modsDir}");
        if (dir.StartsWith(_modsDir!)) {
          foreach (var mod in Mods.Values) {
            Debug.WriteLine($"Checking Context Directory: {dir} vs. {mod.Dir}");
            if (dir.Equals(mod.Dir, StringComparison.OrdinalIgnoreCase)) {
              Debug.WriteLine($"Picked Context Directory: {mod.Dir}");
              ctx = mod;
            }
          }
        }
      ...
      if (ctx is not null) {
        Debug.WriteLine($"Context Directory: {ctx.Dir}");
        ...
        Debug.WriteLine($"Current context is not set");

      Debug.WriteLine($"Current context is missing {name}");
      // check other mods
      foreach (var mod in Mods.Values) {
        if (mod == ctx)
          continue;
